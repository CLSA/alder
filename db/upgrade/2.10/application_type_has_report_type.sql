DROP PROCEDURE IF EXISTS patch_application_type_has_report_type;
  DELIMITER //
  CREATE PROCEDURE patch_application_type_has_report_type()
  BEGIN

    -- determine the @cenozo database name
    SET @cenozo = (
      SELECT unique_constraint_schema
      FROM information_schema.referential_constraints
      WHERE constraint_schema = DATABASE()
      AND constraint_name = "fk_access_site_id"
    );

    SELECT "Removing the analysis report from Salix" AS "";

    SET @sql = CONCAT(
      "DELETE FROM ", @cenozo, ".application_type_has_report_type ",
      "WHERE application_type_id = ( ",
        "SELECT id FROM ", @cenozo, ".application_type WHERE name = 'salix' ",
      ") ",
      "AND report_type_id = ( ",
        "SELECT id FROM ", @cenozo, ".report_type WHERE name = 'analysis' ",
      ") "
    );
    PREPARE statement FROM @sql;
    EXECUTE statement;
    DEALLOCATE PREPARE statement;

    SELECT "Adding records to application_type_has_report_type table" AS "";

    SET @sql = CONCAT(
      "INSERT IGNORE INTO ", @cenozo, ".application_type_has_report_type( application_type_id, report_type_id ) ",
      "SELECT application_type.id, report_type.id ",
      "FROM ", @cenozo, ".application_type, ", @cenozo, ".report_type ",
      "WHERE application_type.name = 'alder' ",
      "AND report_type.name = 'analysis'"
    );
    PREPARE statement FROM @sql;
    EXECUTE statement;
    DEALLOCATE PREPARE statement;

  END //
DELIMITER ;

-- now call the procedure and remove the procedure
CALL patch_application_type_has_report_type();
DROP PROCEDURE IF EXISTS patch_application_type_has_report_type;
