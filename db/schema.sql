...
-- -----------------------------------------------------
-- Table `alder`.`code_type`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `alder`.`code_type` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(),
...
CREATE TABLE IF NOT EXISTS `alder`.`review` (
  ...
  `exam_id` INT(10) UNSIGNED NOT NULL,
  `user_id` INT(10) UNSIGNED NOT NULL,
  `apex_host_id` INT(10) UNSIGNED NULL DEFAULT NULL,
  `apex` TINYINT(1) NOT NULL DEFAULT 0,
  ...
  CONSTRAINT `fk_review_user_id`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_review_apex_host_id`
...
DELIMITER $$
USE `alder`$$
CREATE DEFINER = CURRENT_USER TRIGGER `alder`.`code_AFTER_INSERT` AFTER INSERT ON `code` FOR EACH ROW
BEGIN
  CALL calculate_rating(NEW.analysis_id);
END$$

USE `alder`$$
CREATE DEFINER = CURRENT_USER TRIGGER `alder`.`code_AFTER_DELETE` AFTER DELETE ON `code` FOR EACH ROW
BEGIN
