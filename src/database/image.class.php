...
{
  /**
   * Returns a list of all codes for this image
   */
  public function get_codes( $db_user )
  {
    $code_list = [];
    $db_scan_type = $this->get_exam()->get_scan_type();
    ...
    foreach( $db_scan_type->get_code_group_object_list( $code_group_mod ) as $db_code_group )
    {
      ...
      $join_mod->where( 'code.image_id', '=', $this->id );
      $join_mod->where( 'code.user_id', '=', $db_user->id );
