...
abstract class base_review_post extends \cenozo\service\post
{
  ...
  protected function execute()
  {
    ...
    if( array_key_exists( 'start_date', $file ) )
    {
      ...
      $interview_sel->add_table_column( 'study_phase', 'name', 'study_phase' );
      if( 'apex_review' == $review_type )
      {
        $interview_sel->add_table_column( 'scan_type', 'id', 'scan_type_id' );
        $interview_sel->add_column(
      ...
      {
        $interview_sel->add_table_column( 'modality', 'name', 'modality' );
        $interview_sel->add_table_column( 'site', 'name', 'site' );
        $interview_sel->add_table_column( 'exam', 'interviewer' );
      }

      if( $process )
      {
        $interview_sel->add_column( 'GROUP_CONCAT(DISTINCT interview.id)', 'interview_id_list', false );
      }
      else
      {
        $interview_sel->add_column( 'COUNT(DISTINCT interview.id)', 'total', false );
      }
      $interview_mod = clone $modifier;
      $interview_mod->join( 'site', 'interview.site_id', 'site.id' );
      $interview_mod->join( 'study_phase', 'interview.study_phase_id', 'study_phase.id' );
      $interview_mod->join( 'modality', 'scan_type.modality_id', 'modality.id' );
      $interview_mod->group( 'study_phase.id' );

      if( 'apex_review' == $review_type )
      {
        $interview_mod->group( 'scan_type.id' );
        $interview_mod->order( 'study_phase.name' );
        $interview_mod->order( 'scan_type.name' );
        $interview_mod->order( 'scan_type.side' );
        $interview_mod->where( 'modality.name', '=', 'dxa' );

        // only include exams without an existing review
        $interview_mod->left_join( 'apex_review', 'exam.id', 'apex_review.exam_id' );
        $interview_mod->where( 'apex_review.id', '=', NULL );
      }
      else
      {
        $interview_mod->group( 'modality.id' );
        $interview_mod->group( 'exam.interviewer' );
        $interview_mod->order( 'study_phase.name' );
        $interview_mod->order( 'modality.name' );
        $interview_mod->order( 'site.name' );
        $interview_mod->order( 'exam.interviewer' );
      }

      if( !is_null( $start_date ) )
        $interview_mod->where( 'DATE(CONVERT_TZ(exam.datetime,"UTC",site.timezone))', '>=', $start_date );
      if( !is_null( $end_date ) )
        $interview_mod->where( 'DATE(CONVERT_TZ(exam.datetime,"UTC",site.timezone))', '<=', $end_date );
    ...
    // SELECTION TYPE IS UID BASED
    else if( array_key_exists( 'uid_list', $file ) )
    {
      // prepend a join to the interview table since we're selecting from the participant table
      $modifier->join( 'interview', 'participant.id', 'interview.participant_id', '', NULL, true );
