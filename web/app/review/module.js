  ...
  create: (module) => {
    angular.extend(module, {
      ...
      columnList: {
        ...
        start_datetime: {
          column: "review.start_datetime",
          title: "Start Date & Time",
          type: "datetime",
        },
        end_datetime: {
          column: "review.end_datetime",
          title: "End Date & Time",
          type: "datetime",
        },
        notification: {
          ...
          angular.extend(this, {
              ...
              try {
                ...
                await Promise.all(
                  this.analysisList.map(async (analysis) => {
                    ...
                    analysis.codeGroupList.forEach(
                      g => g.code_list.map(c => { c.working = false; return c })
                    );
                  })
                );
            ...
            selectionChanged: async function(selection) {
              ...
              try {
                const response = await CnHttpFactory.instance({
                  path:
                    "analysis_selection/analysis_id=" + this.currentAnalysis.analysisId +
              ...
              try {
                ...
                if (selection.selection_option_id) {
                  if (null == analysisSelectionId) {
                    ...
                    await CnHttpFactory.instance({
                      path: "analysis_selection/" + analysisSelectionId,
                      data: { selection_option_id: selection.selection_option_id }
                    }).patch();
                  }
                } else {
                  if (null != analysisSelectionId) {
                    await CnHttpFactory.instance({ path: "analysis_selection/" + analysisSelectionId }).delete();
                  }
                }
              } catch (error) {
